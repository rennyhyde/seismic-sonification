...
# Generate MIDI
mf = MIDIFile(numTracks=1, removeDuplicates=False, deinterleave=False)

for index, row in df.iterrows():
    mf.addNote(0, 1, row['along-strike-id(1-88)'], row['playback-time'].total_seconds(), (whole/4), 127)   

# print(mf)
with open("output_direct.mid", 'wb') as outf:
    mf.writeFile(outf)